...
public class Automate {
    ...
    public void display() {
        ...
        String isDeterministe = isDeterministe() ? "est deterministe" : "n'est pas deterministe";
        System.out.println("L'automate " + isDeterministe);
        String isComplet = isComplet() ? "est complet" : "n'est pas complet";
    ...
    public boolean isDeterministe() {
        ...
        for (List<Transition> transitions : E.values()) {
            ...
            if (transitions.stream().collect(Collectors.groupingBy(Transition::getSymbole, Collectors.counting()))
                    .entrySet().stream().anyMatch(occurrence -> occurrence.getValue() > 1))
                return false;
        }
        ...
        for (Transition transition : transitions) {
            ...
            for (String symbole : A) {
                ...
                newTransitions.stream().filter(t -> t.getSymbole().equals(symbole))
                        ...
                        .collect(Collectors.toSet()).stream().map(Transition::getCible).collect(Collectors.toSet())
                        .forEach(sb::append);
                ...
                if (sb.length() > 0) {
                    ...
                    if (automateDeterminise.containsKey(etatCible) && etatCourant.equals(etatCible)) {
                        ...
                        if (automateDeterminise.get(etatCible).stream().anyMatch(t -> t.getCible().equals(etatCible)) && automateDeterminise.get(etatCible).stream().anyMatch(t -> t.getSymbole().equals(symbole)))
                            break;
                    }
    ...
    public boolean isComplet() {
        for (Map.Entry<Etat, List<Transition>> transition : E.entrySet()) {
            for (String symbole : A) {
                if (transition.getValue().stream().noneMatch(t -> t.getSymbole().equals(symbole)))
                    return false;
            }
        }
        ...
        for (Etat etat : T) {
            ...
            for (String symbole : A) {
                if (E.get(etat).stream().noneMatch(transition -> transition.getSymbole().equals(symbole)))
                    return false;
            }
        }
    ...
    public boolean reconnaitLeMot(List<Transition> currentTransitions, String mot) {
        ...
        if (currentTransitions.isEmpty()) {
            for (Etat etat : T) {
                if (E.containsKey(etat))
                    return E.get(etat).stream().map(Transition::getSymbole).collect(Collectors.toList()).contains(character);
            }
        }
    ...
    public void langageComplementaire() {
        ...
        if (!notPresentInitiaux.isEmpty())
            notTerminal.addAll(notPresentInitiaux);
        if (!notPresentTerminaux.isEmpty())
            notTerminal.addAll(notPresentTerminaux);

        T.clear();
        T.addAll(notTerminal);
    }
